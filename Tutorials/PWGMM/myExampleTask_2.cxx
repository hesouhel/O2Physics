...
struct myExampleTask {
  ...
  Preslice<aod::Tracks> perCollision = aod::track::collisionId; //for the track grouping in simulated
  ...
  void init(InitContext const&)
  {
    ...
    const AxisSpec axisReco{10, 0, 10, ""};
  ...
  void processReconstructed(aod::Collision const& collision, myFilteredTracks const& tracks, aod::McParticles const&)
  {
  ...
  PROCESS_SWITCH(myExampleTask, processReconstructed, "process pure simulation information", true);

  //void processSimulated(aod::McParticles const& McParticles)
  void processSimulated(aod::McCollision const& mcCollision, soa::SmallGroups<soa::Join<aod::McCollisionLabels,aod::Collisions>> const& collisions, aod::McParticles const& mcParticles, myFilteredTracks const& tracks)
  { //collision is an iterator (possible only in the first argument) which means it will all be grouped by mccollision id

    histos.fill(HIST("numberOfRecoCollisions"), collisions.size()); // number of times MC collision was reconstructed
