...
struct myExampleTask {
  ...
  void init(InitContext const&)
  {
    ...
    const AxisSpec axisPt{nBinsPt, 0, +10, "#pt"};
  ...
  void process(aod::Collisions const& collisions, myFilteredTracks const& tracks)
  {
    ...
    //for (auto& track : rightTracksThisCollision) {
  ...
  // For manual sliceBy to find if a track is actually ambiguous
  Preslice<aod::AmbiguousTracks> ambiTrackPreslice = o2::aod::ambiguous::trackId;

  void process2(myFilteredTracks const& tracks, o2::aod::AmbiguousTracks const& ambiTracks)
  {
    for (const auto& track : tracks) {
      if( track.tpcNClsCrossedRows() < 70 ) continue;
      // check if ambiguous entry exists
  ...
  PROCESS_SWITCH(myExampleTask, process2, "Do processing two", false);
};
...
WorkflowSpec defineDataProcessing(ConfigContext const& cfgc)
{
  return WorkflowSpec{
    ...
    adaptAnalysisTask<AmbiFiltered>(cfgc),
